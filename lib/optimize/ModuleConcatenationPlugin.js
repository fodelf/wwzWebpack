...
class ModuleConcatenationPlugin {
		...
		compiler.hooks.compilation.tap(
            ...
            // math.count()
              ...
              // try to add all imports
                ...
                let hasProblem = failureCache.get(imp);
                if (hasProblem) {
                  // math.count()
									currentConfiguration.addWarning(imp, hasProblem);
                }
                else{
    ...
    // math.countFor();
      ...
      // math.count()
		...
		for (const reason of module.reasons) {
			...
			if (
				...
				continue;
      let hasProblem = failureCache.get(reason.module);
      if (hasProblem) {
        // math.count()
        return hasProblem;
      }else{
      ...
      let hasProblem = failureCache.get(module);
      if (hasProblem) {
        // math.count()
        config.addWarning(imp, hasProblem);
      }
      else{
        const problem = this._tryToAdd(
          compilation,
